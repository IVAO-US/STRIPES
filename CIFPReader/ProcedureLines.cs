	...
	int FileRecordNumber, int Cycle) : RecordLine(Client, Header, FileRecordNumber, Cycle)
{
	...
	public static (PathTermination, IProcedureEndpoint? ep, IProcedureVia? via, UnresolvedWaypoint? navaidReference) GetPathSegment(string line)
	{
	...
	int FileRecordNumber, int Cycle) : ProcedureLine(Client, "PD", Airport, Name, FileRecordNumber, Cycle)
{
	public static new SIDLine Parse(string line)
	{
		...
		(PathTermination pathTerm, IProcedureEndpoint? ep, IProcedureVia? via, UnresolvedWaypoint? referencePoint) = GetPathSegment(line);
		...
		static AltitudeMSL? getAlt(string data) =>
			...
			? null
			: data[0..2] == "FL"
			  ? new FlightLevel(int.Parse(data[2..^2]))
			  : new AltitudeMSL(int.Parse(data));
	...
	int FileRecordNumber, int Cycle) : ProcedureLine(Client, "PE", Airport, Name, FileRecordNumber, Cycle)
{
	public static new STARLine Parse(string line)
	{
		...
		(PathTermination pathTerm, IProcedureEndpoint? ep, IProcedureVia? procVia, UnresolvedWaypoint? referencePoint) = GetPathSegment(line);
