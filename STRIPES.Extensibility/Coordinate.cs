...
public record struct NamedCoordinate(string Name, Coordinate Position) : ICoordinate
{
...
public record struct Coordinate(decimal Latitude, decimal Longitude) : ICoordinate
{
	public readonly Coordinate GetCoordinate() => this;

	public readonly Coordinate Normalised() =>
		Latitude is >= -90 and <= 90 && Longitude is >= -180 and <= 180
		? this
		: new(decimal.Clamp(Latitude, -90, 90), (Longitude + 540) % 360 - 180);

	/// <summary>
	/// Get the top-left coordinate from a tile's xyz.
	/// </summary>
	public static Coordinate TileTopLeft((int Zoom, int X, int Y) tileCoord) => TileTopLeft(tileCoord.Zoom, tileCoord.X, tileCoord.Y);

	/// <summary>
	/// Get the top-left coordinate from a tile's xyz.
	/// </summary>
	public static Coordinate TileTopLeft(int tileZoom, int tileX, int tileY)
	{
		// tileSpan is the number of tiles to make a strip across the world at the specified zoom.
		int tileSpan = 1 << tileZoom;

		// Get the %age of the way across the world for the tile's top-left.
		decimal tileXuv = tileX / (decimal)tileSpan,
				tileYuv = tileY / (decimal)tileSpan;

		return new(
			(1 - tileYuv * 2) * Quadtree.WEB_MERCATOR_MAX_LAT,
			tileXuv * 360 - 180
		);
	}
	...
	[DebuggerStepThrough]
	public readonly Coordinate RadialIntersect(Course bearing, ICoordinate radialStation, Course radial, decimal tolerance = 0.5m)
	{
		if (radialStation.GetCoordinate().GetBearingDistance(this).bearing is null)
			return this;
		...
		void stepTowards()
		{
			var data = radialStation.GetCoordinate().GetBearingDistance(exploratory);
			if (data.bearing is null)
				throw new Exception();

			var stepSize = data.distance * Math.Abs(data.bearing.Angle(radial));

			var guess = exploratory.FixRadialDistance(bearing, stepSize);
			if (radialStation.GetCoordinate().DistanceTo(guess) < data.distance)
				exploratory = guess;
			else
				exploratory = exploratory.FixRadialDistance(bearing, -stepSize);
		}

		while (radialStation.GetCoordinate().GetBearingDistance(exploratory).bearing is Course newBearing && Math.Abs(radial.Angle(newBearing)) > tolerance)
			stepTowards();
	...
	public override readonly string ToString() => $"({Latitude:00.000}, {Longitude:000.000})";
